...
public:
    ...
    inline void SqlStatement(const std::string &sqlStatement)
    {
        SqlStatement(sqlStatement.c_str());
    }
    //! Only for SQL statements which have no result (e.g. INSERT, UPDATE, etc).\n
    //! Can be used for transactions; if you want, you can use an own error handling.\n
    //! Please use 1 (true) and 0 (false) as bool values instead of 'true' and 'false' as string.\n
    ...
    inline void SqlStatement(const std::wstring &sqlStatement)
    {
        SqlStatement(sqlStatement.c_str());
    }
    //! Only for SQL statements which have no result (e.g. INSERT, UPDATE, etc).\n
    ...
    inline void Sql(const std::string &sql)
    {
        Prepare(sql.c_str());
    }
    //! Only for SQL queries/statements which have a result or for prepared statements.\n
    //! e.g. SELECT's or INSERT's which use Bind..() methods!\n
    //! Do not forget to call FreeQuery() when you have finished.
    inline void Sql(const char *sql)
    {
        Prepare(sql);
    }
    //! Only for SQL queries/statements which have a result or for prepared statements.\n
    //! e.g. SELECT's or INSERT's which use Bind..() methods!\n
    //! Do not forget to call FreeQuery() when you have finished.
    inline void Sql(const std::wstring &sql)
    {
        Prepare(sql.c_str());
    }
    //! Only for SQL queries/statements which have a result or for prepared statements.\n
    //! e.g. SELECT's or INSERT's which use Bind..() methods!\n
    //! Do not forget to call FreeQuery() when you have finished.
    inline void Sql(const wchar_t *sql)
    {
        Prepare(sql);
    }
    ...
    inline void Transaction(const char *sql)
    {
        mTransactionSQL[mTransactionID++] = std::make_pair(sql, false);
    }
    //! Can be used only for transaction SQL statements.\n
    //! Can be used for transactions, if you want use the default error handling.
    //! Please note that there is only used a reference of your sql statement.\n
    ...
    //! you need to use SecureTransaction(), which creates a internal copy of your sql statement.
    //! @param sql		SQL statement
    inline void Transaction(const std::string &sql)
    {
        mTransactionSQL[mTransactionID++] = std::make_pair(sql.c_str(), false);
    }
    //! Can be used only for transaction SQL statements.\n
    //! Can be used for transactions, if you want use the default error handling.
    //! Please note that there is only used a reference of your sql statement.\n
    ...
    //! you need to use SecureTransaction(), which creates a internal copy of your sql statement.
    //! @param sql		SQL statement
    inline void Transaction(const wchar_t *sql)
    {
        mTransactionSQL16[mTransactionID++] = std::make_pair(sql, false);
    }
