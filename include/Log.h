...
public:
	...
	static void err(const char* fmt, ... ){ FMTPARCE syslog(LOG_ERR, "%s", buffer);};
	static void warn(const char* fmt, ... ){ FMTPARCE syslog(LOG_WARNING, "%s", buffer);};
	static void info(const char* fmt, ... ){ FMTPARCE syslog(LOG_INFO, "%s", buffer);};
